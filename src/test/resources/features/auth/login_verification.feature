...
Característica: Verificación de Login de Usuario
  ...
  @smoke @regression @api:API-07 @issue:7 @requires_user
  ...
  @regression @negative @api:API-08 @issue:8
  Escenario: [API-08] [POST /api/verifyLogin] Intentar verificación de login sin proporcionar el parámetro contraseña
    Dado que el actor está listo para consumir la API de Automation Exercise
    Cuando el actor verifica el login sin proporcionar la contraseña con email "transunion.automation.test@gmail.com"
    Entonces el mensaje de respuesta debe ser "Bad request, email or password parameter is missing in POST request."
    Y el código de respuesta en el cuerpo debe ser 400
    Y el cuerpo de la respuesta debe coincidir con el esquema JSON "schemas/common/api_response_schema.json"
